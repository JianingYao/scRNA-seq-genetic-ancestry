...
$PLINK2 --bfile TGP_PLINK_${POP}.${SNP}_pruned  --pca --within TGP_PLINK.no$POP.cluster --pca-cluster-names TGP --out TGP_PLINK.${SNP}_pca.no$POP
...
cp TGP_PLINK_${POP}.${SNP}_pruned.bed  TGP_PLINK.${SNP}_admixture.no$POP.bed
cp TGP_PLINK_${POP}.${SNP}_pruned.bim  TGP_PLINK.${SNP}_admixture.no$POP.bim
cp TGP_PLINK_${POP}.${SNP}_pruned.fam  TGP_PLINK.${SNP}_admixture.no$POP.fam
/project2/gazal_569/jianing/sc-RNA_ancestry/bin/admixture_linux-1.3.0/admixture --supervised TGP_PLINK.${SNP}_admixture.no$POP.bed 6
