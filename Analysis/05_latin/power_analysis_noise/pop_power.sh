...
$PLINK2 --bfile TGP_PLINK_$POP.${SNP}_pruned --remove $POP.$SNP.list --make-bed --out $POP.$SNP.tmp1
$PLINK2 --bfile TGP_PLINK_$POP.${SNP}_pruned --keep   $POP.$SNP.list --recode transpose --out $POP.$SNP.tmp2
Rscript /project2/gazal_569/jianing/sc-RNA_ancestry/scripts/Analysis/05_latin/power_analysis_noise/add_noise_seed.r $POP.$SNP $ERROR
