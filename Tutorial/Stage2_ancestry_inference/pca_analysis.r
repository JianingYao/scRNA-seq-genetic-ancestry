...
data=read.table(paste0(DATA_DIR,"/", STUDY,"/",STUDY,".HGDP_1kGP.pca.eigenvec"),h=F)
...
split.screen(c(1,2))
screen(1); plot(data_HGDP_1kGP$V3,data_HGDP_1kGP$V4 ,col=as.character(info$Color),pch=info$PCH,xlab="PC1",ylab="PC2",xlim=c(min(x1),max(x1)),ylim=c(min(y1),max(y1))); points(mydata$V3,mydata$V4,pch=16)
screen(2); plot(data_HGDP_1kGP$V5,data_HGDP_1kGP$V6 ,col=as.character(info$Color),pch=info$PCH,xlab="PC3",ylab="PC4",xlim=c(min(x2),max(x2)),ylim=c(min(y2),max(y2))); points(mydata$V5,mydata$V6,pch=16)
...
dataRF = cbind(ancestry,data_HGDP_1kGP[,3:tokeep])
